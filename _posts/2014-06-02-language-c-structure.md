...
19. 다음 네 가지의 경우를 잘 이해하자!  
    <p>- (*p).number : 포인터 p가 가리키는 구조체의 멤버 number</p>  
    <p>- p->number(=(*p).number : 포인터 p가 가리키는 구조체의 멤버 number</p>  
    <p>- *p.number(=*(p.number)) : 구조체 p의 멤버 number가 가리키는 것, 이때 number는 반드시 포인터여야 한다.</p>  
    <p>- *p->number(=*(p->number)) : p가 가리키는 구조체의 멤버 number가 가리키는 내용, 이때 number는 반드시 포인터여야 한다.</p>  
