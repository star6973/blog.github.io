...
<p> 1) (*p).number : 포인터 p가 가리키는 구조체의 멤버 number  
    2) p->number(=(*p).number : 포인터 p가 가리키는 구조체의 멤버 number  
    3) *p.number(=*(p.number)) : 구조체 p의 멤버 number가 가리키는 것, 이때 number는 반드시 포인터여야 한다.  
    4) *p->number(=*(p->number)) : p가 가리키는 구조체의 멤버 number가 가리키는 내용, 이때 number는 반드시 포인터여야 한다.</p>  
<br>
20. 연결리스트
    - 자기 자신을 가리키는 포인터
