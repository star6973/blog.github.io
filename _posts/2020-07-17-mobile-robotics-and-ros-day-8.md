...
- Motion Planning Formulation
    + 주어진 것
        + 로봇의 시작 위치, 원하는 목표, 로봇의 기하학적 설명, 환경의 기하학적 표현
        + 장애물을 건드리지 않고 로봇을 처음부터 목표로 점차 이동시키는 경로를 찾기
...
-  Configuration Space
    ...
    - Search  
        ...
        2) Informed search
        + 어느정도 길은 안다(어느 방향으로 가야될 지에 대한 정보[휴리스틱 정보]가 주어짐) - 그리디 알고리즘, A*, Memory-Bounded
        + 가장 전도유망한 방향으로 search를 진행
        
        > [Informed Search(Heuristic Search) 설명과 예시](https://m.blog.naver.com/ndb796/220578642298){:target="_blank"}
